...
public:
    bool CzyWsp;
    AktWsp():Rama(7),CzyWsp(false){};
    void ZmienWsp(){CzyWsp=false;}
    void UstRame(int R){if(Rama<=20 && Rama>0) Rama=R;  } //ograniczenia ramki
    int Xe(double Xrz);
    int Ye(double Yrz);
...
bool TForm3::sprawdz()
{
  ...
  for(int i=0;i<3;i++ )
  {
      ...
      for(int i=startIndeks+1;i<tym->indeks;i++)
        {
         I =DajY(i, NumerPliku);
         I=fabs(I);
         Amp= fabs(I/I01);
        ...
        for(int i=tym->indeks+1;i<stopIndeks;i++)
        {
         I =DajY(i, NumerPliku);
         I=fabs(I);
         Amp= fabs(I/I02);
         ...
         TabKsztalt_L[i]=(((Gaus-Amp)*(Gaus-Amp)) > ((Lor-Amp)*(Lor-Amp)))?true:false;
         Tab[0][i]=(B-B0)*(B-B0);
         if(I==0) I=0.00000000000000000000001;
         Tab[1][i]=sqrt((B-B0)/I);
         Tab[2][i]=log(I/(B-B0));
        }
        Tab[0][0]=B0;
