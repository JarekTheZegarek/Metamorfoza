...
const crMyCursor=5;
const crCursorMagnPlus = 20;
const crCursorMagnMinus = 21;
void __fastcall TForm1::FormCreate(TObject *Sender)
{
  ...
  char* plik1="magnify_minus.cur";
  Screen->Cursors[crCursorMagnMinus] =LoadImage(0,plik1,IMAGE_CURSOR,0,0,LR_LOADFROMFILE); //LoadCursor(MainInstance, "CURSORMAGMINUS");
  char* plik2="magnify_plus.cur";
  Screen->Cursors[crCursorMagnPlus] =LoadImage(0,plik2,IMAGE_CURSOR,0,0,LR_LOADFROMFILE); //LoadCursor(MainInstance, "CURSORMAGPLUS");
  Image1->Cursor =static_cast<TCursor>(crMyCursor);
...
try{
  ...
  TPoint PunktWykr = AktWykrLinii->DajWsp(0,DajY(0,NumerPliku));
  MBit->Canvas->MoveTo(PunktWykr.x,PunktWykr.y);
  if(Opcje.PokazPunkty)
        for(int i=1;i<Akt->LiczWierszy();i++)
        {
         PunktWykr= AktWykrLinii->DajWsp(i,DajY(i,NumerPliku));
         Doswiadczalny->Rysuj(MBit->Canvas,PunktWykr.x,PunktWykr.y);
         MBit->Canvas->LineTo(PunktWykr.x,PunktWykr.y);
        }
  else
        for(int i=1;i<Akt->LiczWierszy();i++)
        {
 ...
 switch(Tryb)
  {
   ...
   case Nieokreslony:
            ...
            Punkt* P;
            while( (P=ZebranePunkty.DajNastepny(NumerPliku))!= 0 )
            {
                PunktWykr =  AktWykrLinii->DajWsp(P->DajIndeks(),DajY(P->DajIndeks(),NumerPliku));
                Zaznaczenie->Rysuj(MBit->Canvas,PunktWykr.x,PunktWykr.y);
                ObszarPunkty::Przejscie* Prz_ptr;
                ObszarPunkty::Punkt::TIntIter it;
                for(it=P->Przejscia.begin();it!=P->Przejscia.end();it++)
                {
                        NrPrzej = *it;
                        Prz_ptr = ListaPrzejsc.DajPrzejscie(NrPrzej);
                        Sym=Prz_ptr->DajKsztalt();
                        Sym->Rysuj(MBit->Canvas, PunktWykr.x, PunktWykr.y);
                }
            }
         /*
            ...
            while(tym)         // Odœwie¿anie zaznaczonych punktów
            {
               ...
               PunktWykr= AktWykrLinii->DajWsp(tym->indeks,DajY(tym->indeks,NumerPliku));
               Zaznaczenie->Rysuj(MBit->Canvas,PunktWykr.x,PunktWykr.y);
               tym=tym->nastepny;
            }
            */
   ...
   case ZaznaczanieSzerokosci:
            ...
            if(BB.x>=0 && BB.y >=0)
            {
                ...
                PunktWykr= AktWykrLinii->DajWsp(X1S,DajY(X1S,NumerPliku));
                MBit->Canvas->MoveTo(PunktWykr.x,PunktWykr.y);
                for(int i=X1S; i< X2S;     i++)
                {

                 PunktWykr= AktWykrLinii->DajWsp(i,DajY(i,NumerPliku));
                 if(Opcje.PokazPunkty) Doswiadczalny->Rysuj(MBit->Canvas,PunktWykr.x,PunktWykr.y);
                 MBit->Canvas->LineTo(PunktWykr.x,PunktWykr.y);
                }
                AktWykrLinii->UstawKolorLinii(clRed);
            ...
            if(BB.x>=0)
             { PunktWykr= AktWykrLinii->DajWsp(BB.x,DajY(BB.x,NumerPliku));
              Zaznaczenie->Rysuj(MBit->Canvas,PunktWykr.x,PunktWykr.y);}
            if(BB.y>=0)
             { PunktWykr= AktWykrLinii->DajWsp(BB.y,DajY(BB.y,NumerPliku));
              Zaznaczenie->Rysuj(MBit->Canvas,PunktWykr.x,PunktWykr.y);
              }
             if(BB.x <0 || BB.y <0)
             {
    ...
    case mbLeft:
         {
           ...
           {
             double Wart=Konwertuj(Akt->DajWartosc(NumerPliku));
             Punkt P(NumerPliku,Indeks,Wart,DajX(Indeks,NumerPliku));
    ...
    case mbRight:
         {
           ...
           if((Indeks=SzukajNajblIndeksu(Akt,X,Y))>=0)
           {
             P_ptr = ZebranePunkty.ZnajdzPunkt(
                     Akt->DajWartosc(NumerPliku),DajX(Indeks,NumerPliku),NumerPliku);
             if(P_ptr)    // Je¿eli znaleziono punkt ;
                 ZebranePunkty.UsunPunkt(*P_ptr);
           }
...
if(!rysunek)
{
 for(int i=1; i<=LiczbaWczytPlikow;i++)
 {
   ...
   if((tym=Akt->Pola[Akt->DajIndeks(i)].nastepny)!=0)
   {
...
{
     enum TypyPlikow {Matematika=1,PlikiDat=2,PlikiSzerDat=3,PlikiIntens=5,InnePliki=4} TypPliku;
//     enum {Matematika=1,PlikiDat=2,PlikiSzerDat=3,InnePliki=4} TypPliku;
     ...
     EksportDialog->FilterIndex=Matematika;
      if(EksportDialog->Execute())
      {
       ...
       TypPliku=static_cast<TypyPlikow>(EksportDialog->FilterIndex);
...
void __fastcall TForm1::ToolButton1Click(TObject *Sender)
{
  ZbiorTablic* Akt=DajZb(NumerPliku);
  Punkt* P_ptr;
  while((P_ptr=ZebranePunkty.DajNastepny(NumerPliku))!=0)
           ZebranePunkty.UsunPunkt(*P_ptr);
...
void __fastcall TForm1::WykresPunkty1Click(TObject *Sender)
{
 ...
 LiczbaZebranychPolozen=ZebranePunkty.DajLiczbePunktow();
...
{
  ZapisSesji(2);   // Funkcja znajduje siê w zbiorze ZapisOdczytSesji.cpp
...
{
    if (MessageDlg("Czy napewno wyjœæ z programu?", mtConfirmation, TMsgDlgButtons() << mbOK << mbCancel,0) == mrCancel)
    CanClose = false;
